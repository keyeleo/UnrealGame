...
TArray<const AVCodec*>  FFFMPEGMediaTracks::FindDecoders(int codecId, bool hwaccell) {
    ...
    if (candidates.Num() > 0) {
        if (!hwaccell) {
            for (const AVCodec* pcodec : candidates) {
                if (!isHwAccel(pcodec)) {
                    codecs.Add(pcodec);
                }
            }
...
int FFFMPEGMediaTracks::StreamComponentOpen(int stream_index) {
    ...
    if ( !codec) {
		...
		FString Msg = TEXT("audio"); 
		if (desc)
		{
			Msg = UTF8_TO_TCHAR(desc->long_name);
		}
        UE_LOG(LogFFMPEGMedia, Error, TEXT("coudn't find a decoder for %s"), *Msg);
        return -1;
    }
...
int FFFMPEGMediaTracks::UploadTexture(FFMPEGFrame* vp, AVFrame *frame, struct SwsContext **img_convert_ctx) {
    ...
    FTimespan tDuration = FTimespan::FromSeconds(vp->GetDuration());
                ...
                if (subpq.GetNumRemaining() > 0) {
                    ...
                    if (vp->GetPts() >= sp->GetPts() + ((float)sp->GetSub().start_display_time / 1000)) {
                        if (!sp->IsUploaded()) {
                            ...
                            FTimespan tDuration = FTimespan::FromSeconds(sp->GetDuration());
                            ...
                            for (i = 0; i < (int)sp->GetSub().num_rects; i++) {
                                ...
                                if ( sub_rect->type == SUBTITLE_TEXT ||  sub_rect->type == SUBTITLE_ASS) {
                                    ...
                                    if (CaptionSample->Initialize(sub_rect->type == SUBTITLE_TEXT?sub_rect->text:sub_rect->ass, FVector2D(sub_rect->x, sub_rect->y), Time, tDuration))
                                    {
...
int FFFMPEGMediaTracks::ReadThread() {
    ...
    for (;;) {
		...
		TargetPlayer->reading = true;
		TargetPlayer->lastReadingTime = FDateTime::Now();
...
int FFFMPEGMediaTracks::AudioDecodeFrame(FTimespan& Time, FTimespan& tDuration) {
	...
	tDuration = FTimespan::FromSeconds(af->GetDuration());
...
void FFFMPEGMediaTracks::RenderAudio() {
    ...
    FTimespan Time = 0;
    FTimespan tDuration = 0;

    audio_size = AudioDecodeFrame(Time, tDuration);
    if (audio_size < 0) {
    ...
    if (CurrentState == EMediaState::Paused || CurrentState == EMediaState::Stopped) {
        ...
        if ( audioBuf != NULL ) {
            ...
            if (AudioSample->Initialize((uint8_t *)audioBuf, len1, targetAudio.NumChannels, targetAudio.SampleRate, Time, tDuration))
            {
